...
export const Language: React.FC<LanguageProps> = ({ onNext }) => {
  ...
  const handleAdd = () => {
    setLanguageList(list => [...list, { id: nextId, language: '', proficiency: '' }]);
    setNextId(nextId + 1);
  };

  const handleDelete = (id: number) => {
    setLanguageList(list => list.filter(entry => entry.id !== id));
  };

  const allFilled = languageList.every(
      ...
      <div className="px-6 flex flex-col gap-6">
        {languageList.map((entry) => (
          <div key={entry.id} className="grid grid-cols-2 gap-6 relative">
            <CustomDropdown
              label={<span>Languages spoken <span style={{color:'#E6483D'}}>*</span></span>}
              options={languageOptions}
              required
              value={entry.language}
              onChange={val => handleChange(entry.id, 'language', val)}
            />
            <CustomDropdown
              label={<span>Proficiency level <span style={{color:'#E6483D'}}>*</span></span>}
              options={proficiencyOptions}
              required
              value={entry.proficiency}
              onChange={val => handleChange(entry.id, 'proficiency', val)}
            />
