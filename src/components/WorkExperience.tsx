...
export const WorkExperience: React.FC = () => {
  ...
  const handleAdd = () => {
    setWorkList(list => [...list, { id: nextId, jobTitle: '', designation: '', company: '', from: '', to: '', onwards: false }]);
    setNextId(nextId + 1);
  };

  const handleDelete = (id: number) => {
    setWorkList(list => list.filter(entry => entry.id !== id));
  };

  const allFilled = workList.every(
    entry => entry.jobTitle && entry.designation && entry.company && entry.from && (entry.onwards || entry.to)
  );
        ...
        {workList.map((entry) => (
          <div key={entry.id} className="grid grid-cols-2 gap-6 relative">
            <Input
              ...
              onChange={e => handleChange(entry.id, 'jobTitle', e.target.value)}
            />
            <Input
              label="Designation"
            ...
            <div className="col-span-2 flex gap-6 items-end">
              <DateInput
                ...
                required
                value={entry.from}
                onChange={e => handleChange(entry.id, 'from', e.target.value)}
              />
              <DateInput
                label="To"
                required={!entry.onwards}
                value={entry.to}
              ...
              <Checkbox
                ...
                onChange={e => handleChange(entry.id, 'onwards', e.target.checked)}
              />
            </div>
            {workList.length > 1 && (
              <>
